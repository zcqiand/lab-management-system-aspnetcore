#!/bin/sh
# setup-vps.sh — VPS 一次性 bootstrap (Ubuntu/Debian) — lab-management-system-aspnetcore
#
# 用法:
#   sudo sh deploy/setup-vps.sh lab-aspnetcore.example.com [cert-basename]
#
# 同一台 VPS 上要先跑 lab-nextjs/react/vue/springboot 的 setup-vps.sh
...
DOMAIN="${1:-}"
if [ -z "$DOMAIN" ]; then
  echo "Usage: $0 <lab-aspnetcore.example.com>" >&2
  exit 1
fi
...
# aspnetcore 这边重复声明 nginx -t 会直接报错 —— 务必先装 lab 系之一并把 default 占位收掉,
