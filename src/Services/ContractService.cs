...
public sealed class ContractService(IFlowStore store)
{
    ...
    public Contract Create(string tenantId, CreateContractRequest body)
    {
        ...
        var c = new Contract
        {
            ...
            // T11(2026-09-16)：可选字段不再 "" 兜底 —— nextjs(PG null)/springboot(JPA null)
            // 未填可选字段序列化成 null（normalize 剔 null），aspnetcore 唯独吐 "" 导致
            // POST shape 四方比对结构性分叉。SSOT 可选语义 = null，序列化 null 序即对齐。
            ProjectLocation = body.ProjectLocation ?? null!,
            ConstructionUnit = body.ConstructionUnit,
            InspectionSpecialtyCode = body.InspectionSpecialtyCode ?? null!,
            BuildingUnit = body.BuildingUnit ?? null!,
            SupervisorUnit = body.SupervisorUnit ?? null!,
            InspectionPerson = body.InspectionPerson ?? null!,
            InspectionPhone = body.InspectionPhone ?? null!,
            WitnessUnit = body.WitnessUnit ?? null!,
            Witness = body.Witness ?? null!,
            WitnessPhone = body.WitnessPhone ?? null!,
